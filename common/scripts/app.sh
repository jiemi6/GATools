...
JAVA_OPTS_JMX="-ms512m -mx512m -Xmn256m -Djava.awt.headless=true -Dcom.sun.management.jmxremote=true -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false"
#调试参数
debug_port=$2
...
psid=0
checkpid() {
javaps=`ps ax|grep java|grep $APP_CLASS |grep -v grep`
if [ -n "$javaps" ]; then
